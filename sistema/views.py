...
from .serializers import AlumnoSerializer, LibroSerializer    # add this
import base64
import datetime
import operator
import json
...
def alumno_list(request):
    ...
    # https://bezkoder.com/django-postgresql-crud-rest-framework/#Write_API_Views
    if request.method == 'GET':
        Alumnos = Alumno.objects.all()
        getcorreo = request.GET.get('correo', None)
        if getcorreo is not None:
            Alumnos = Alumnos.filter(correo=getcorreo)
...
def alumno_detail(request, pk):
    if request.method == 'GET':
        if pk is not None:
            Alumno_f = Alumno.objects.get(pk=pk)
            Alumno_f.fecha_nacimiento = Alumno_f.fecha_nacimiento.strftime(
                "%d/%m/%Y")
            serializer_class = AlumnoSerializer(Alumno_f)
            return JsonResponse(serializer_class.data)


@api_view(['GET', 'PUT', 'DELETE'])
def grupos_detail(request, pk):
    if request.method == 'GET':
        response = list()
        if pk is not None:
            rel = rel_alu_doc_grupo.objects.filter(id_alumno=pk)
            rel2 = rel_aula_grupo.objects.all()
            grupos = Grupo.objects.all()
            docentes = Docente.objects.all()
            cursos = Curso.objects.all()
            aulas = Aula.objects.all()
            for x in rel:  # rel = grupos a los que pertenece el alumno
                for y in grupos:
                    if x.id_grupo == y.pk:  # sacar la info de los grupos
                        for i in docentes:
                            if i.pk == y.id_docente:
                                for j in cursos:
                                    if j.pk == y.id_curso:
                                        for r in rel2:
                                            if r.id_grupo == y.pk:
                                                for a in aulas:
                                                    if r.id_aula == a.pk:
                                                        grupo = {
                                                            "id": y.pk,
                                                            "nombre": y.nombre,
                                                            "docente": str(i.nombre+" "+i.apellido),
                                                            "curso": str(j.nombre),
                                                            "aula": str(a.nombre_aula),
                                                            "horainicio": str(y.hora_inicio.strftime("%H:%M %p")),
                                                            "horafin": str(y.hora_fin.strftime("%H:%M %p")),
                                                            "fechafin": str(y.fecha_fin.strftime("%d/%m/%Y")),
                                                            "fechainicio": str(y.fecha_inicio.strftime("%d/%m/%Y"))
                                                        }
...
def certificados_list(request, pk):
    if request.method == 'GET':
        response = list()
        if pk is not None:
            certificados = Certificado.objects.filter(id_alumno=pk)
            grupos = Grupo.objects.all()
            cursos = Curso.objects.all()
...
def libros_detail(request):
    if request.method == 'GET':
        ...
        serializer_class = LibroSerializer(libros, many=True)
        return JsonResponse(serializer_class.data, safe=False)

# VIEWS DEL SISTEMA WEB
...
def consultar_a(request):
    ...
    if request.method == 'POST':
        if 'registrar' in request.POST:
            ...
            nuevo_alumno.save()
            nueva_rel = rel_alu_doc_grupo(
                usuarioultmod=usuario, usuarioreg=usuario, id_alumno=nuevo_alumno.pk, id_grupo=grupo_f.pk)
            nueva_rel.save()
            ...
            usuario = request.user.username
            Alumno.objects.filter(pk=iddel).update(
                activo=False, fechaultmod=datetime.datetime.now(), usuarioultmod=usuario)
            return HttpResponseRedirect("/consulta_a/")
        elif 'reactivar' in request.POST:
            idreac = request.POST['reactivar']
            usuario = request.user.username
            Alumno.objects.filter(pk=idreac).update(
                activo=True, fechaultmod=datetime.datetime.now(), usuarioultmod=usuario)
            return HttpResponseRedirect("/consulta_a/")
    return render(request, 'consultar_alumnos.html', Context)


@login_required(login_url='inicio')
def royalAppPassword(request):
    alumnos = Alumno.objects.all()
    Context = {
        'Alumnos': alumnos,
    }
...
def consultar_d(request):
    ...
    if request.method == 'POST':
        if 'registrar' in request.POST:
            ...
            iddel = request.POST['eliminar']
            usuario = request.user.username
            Docente.objects.filter(pk=iddel).update(
                activo=False, fechaultmod=datetime.datetime.now(), usuarioultmod=usuario)
            return HttpResponseRedirect("/consulta_d/")
        elif 'reactivar' in request.POST:
            idreac = request.POST['reactivar']
            usuario = request.user.username
            Docente.objects.filter(pk=idreac).update(
                activo=True, fechaultmod=datetime.datetime.now(), usuarioultmod=usuario)
            return HttpResponseRedirect("/consulta_d/")
...
def certificados(request):
    ...
    if request.method == 'POST':
        ...
        if 'eliminar' in request.POST:
            iddel = request.POST['eliminar']
            certificado = Certificado.objects.get(pk=iddel)
            certificado.archivo.delete(save=True)
            certificado.delete()
            return HttpResponseRedirect("/certificados/")
...
def curso(request):
    ...
    if request.method == 'POST':
        if 'registrar' in request.POST:
            ...
            iddel = request.POST['eliminar']
            usuario = request.user.username
            Curso.objects.filter(pk=iddel).delete()
            return HttpResponseRedirect("/curso/")
...
def modalCursos(request, id=None):
    ...
    grupos = Grupo.objects.all().filter(id_curso=id)
    context = {
        'CursoF': CursoF,
        'Grupos': grupos
    }
...
def aula(request):
    ...
    if request.method == 'POST':
        if 'registrar' in request.POST:
            ...
            iddel = request.POST['eliminar']
            usuario = request.user.username
            Aula.objects.filter(pk=iddel).update(
                activa=False, usuarioultmod=usuario, fechaultmod=datetime.datetime.now())
            return HttpResponseRedirect("/aula/")
...
def modalAulas(request, id=None):
    ...
    rel = rel_aula_grupo.objects.filter(id_aula=id)
    context = {
        'AulaF': AulaF,
        'Rel': rel
    }
...
def promociones(request):
    ...
    if request.method == 'POST':
        if 'registrar' in request.POST:
            ...
            iddel = request.POST['eliminar']
            usuario = request.user.username
            Promocion.objects.filter(pk=iddel).update(
                activa=False, usuarioultmod=usuario, fechaultmod=datetime.datetime.now())
            return HttpResponseRedirect("/promociones/")
...
def grupos(request):
    ...
    if request.method == 'POST':
        if 'registrar' in request.POST:
            ...
            grupo_nuevo.save()
            rel_nueva = rel_aula_grupo(
                usuarioreg=usuario, usuarioultmod=usuario, id_aula=aula, id_grupo=grupo_nuevo.pk)
            rel_nueva.save()
            ...
            iddel = request.POST['eliminar']
            usuario = request.user.username
            Grupo.objects.filter(pk=iddel).update(
                activo=False, usuarioultmod=usuario, fechaultmod=datetime.datetime.now())
            return HttpResponseRedirect("/grupos/")
...
def moverAlumnos(request, id_alumno=None, id_grupo=None, movimiento=None):
    if request.is_ajax():
        ...
        if movimiento == 1:
            ...
            usuario = 'reynoso'
            nueva_rel = rel_alu_doc_grupo(
                usuarioultmod=usuario, usuarioreg=usuario, id_alumno=id_alumno, id_grupo=id_grupo)
            nueva_rel.save()
...
def moverAulas(request, id_aula=None, id_grupo=None, movimiento=None):
    if request.is_ajax():
        ...
        if movimiento == 1:
            ...
            usuario = 'reynoso'
            nueva_rel = rel_aula_grupo(
                usuarioultmod=usuario, usuarioreg=usuario, id_aula=id_aula, id_grupo=id_grupo)
            nueva_rel.save()
    ...
    if request.method == 'POST':
        if 'registrarPago' in request.POST:
            ...
            if fechaPago == '':
                ...
                # Se obtiene ultimo dato de caja para hacer el acumulado
                ultimoRegistroCaja = Caja.objects.latest('id')
    ...
    if request.method == 'POST':
        ...
        for idx, c in enumerate(request.POST.getlist('cantidadP')):
            ...
            if tipoProducto[idx] == 'Producto':
                nuevoDetalleV = Detalle_venta(id_venta=ultimoRegistroDetalleVenta, id_producto=id_producto[idx], tipoProducto=tipoProducto[
                                              idx], cantidad=c, monto=m, usuarioreg=usuario, usuarioultmod=usuario, fechareg=datetime.datetime.now(), fechaultmod=datetime.datetime.now())
                nuevoDetalleV.save()
                ...
                # se obtiene el producto a modificar su inventario
                prueba = Producto.objects.filter(id=id_producto[idx])
                # print(prueba[0].nombre)
                # print('indice:',idx)
                prueba2 = prueba[0]
                prueba2.inventario = float(
                    objInventario[0].inventario) - float(c)
                prueba2.save()
            else:
                nuevoDetalleV = Detalle_venta(id_venta=ultimoRegistroDetalleVenta, id_producto=id_producto[idx], tipoProducto=tipoProducto[
                                              idx], cantidad=c, monto=m, usuarioreg=usuario, usuarioultmod=usuario, fechareg=datetime.datetime.now(), fechaultmod=datetime.datetime.now())
                ...
                # print('indice:',idx)
                prueba2 = prueba[0]
                prueba2.restantes = float(
                    objInventario[0].restantes) - float(c)
                prueba2.vendidos += 1
                prueba2.save()
            nuevoDetalleV.save()
...
def inventario(request):
    ...
    if request.method == 'POST':
        if 'registrar' in request.POST:
            ...
            iddel = request.POST['eliminar']
            usuario = request.user.username
            Producto.objects.filter(pk=iddel).update(
                activo=False, fechaultmod=datetime.datetime.now(), usuarioultmod=usuario)
            return HttpResponseRedirect("/inventario/")
...
def caja(request):
    ...
    if not cajas:
        if 'registrar' in request.POST:
            ...
            usuario = request.user.username
            Caja(cantidad=monto, activa=True, usuarioreg=usuario,
                 usuarioultmod=usuario).save()
        ...
        if caja.fechareg != help.fechaultcaja:
            ...
            if 'cerrar' in request.POST:
                ...
                usuario = request.user.username
                Caja.objects.filter(pk=id).update(
                    usuarioultmod=usuario, fechaultmod=datetime.datetime.now(), activa=False)
                return HttpResponseRedirect("/caja/")
                ...
                # Registro de campos para tabla RegistroCaja, nuevo objeto
                registroNvoCaja = RegistroCaja(
                    tipo='Egreso', monto=cantidadRetiro, motivo='Retiro a caja', usuarioreg=usuario, usuarioultmod=usuario)
                # Se guarda el objeto generado de el nvo registro de caja
                registroNvoCaja.save()
                return HttpResponseRedirect("/caja/")
                ...
                # Registro de campos para tabla RegistroCaja, nuevo objeto
                registroNvoCaja = RegistroCaja(
                    tipo='Ingreso', monto=cantidadIngreso, motivo='Ingreso a caja', usuarioreg=usuario, usuarioultmod=usuario)
                # Se guarda el objeto generado de el nvo registro de caja
                registroNvoCaja.save()
                return HttpResponseRedirect("/caja/")
